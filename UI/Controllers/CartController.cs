...
namespace UI.Controllers
{
    public class CartController : Controller
    {
        ...
        public async Task<ActionResult> CompleteOrderUser(CheckOutUserViewModel model)
        {
            ...
            foreach (var item in cart)
            {
                ...
                var order = new Order
                {
                    ...
                    ProductAmount = item.Quantity,
                    Price = item.Price,
                    AdressTitle = selectedAddressTitle,  // Adresin başlığı (Ev, İş vs.)
                    FirstName = user.FirstName,
                    LastName = user.LastName,
                    Adress = model.SelectedAddress,  // Kullanıcının seçtiği adres
                    Email = user.Email,
        ...
        public JsonResult IncreaseQuantity(int productId)
        {
            ...
            if (userId != null)
            {
                // Kullanıcı login olmuş, Redis'teki sepeti alıyoruz
                cart = _redisHelper.GetCart(userId) ?? new List<CartItem>();
            }
            else
            {
                // Kullanıcı login olmamış, Session'dan sepeti alıyoruz
                cart = SessionHelper.Get<List<CartItem>>(HttpContext.Session, "Cart") ?? new List<CartItem>();
            }
            ...
            {
                cartItem.Quantity++;
                cartItem.Price = cartItem.UnitPrice * cartItem.Quantity; // Yeni fiyatı hesapla

                // Güncellenmiş cart'u Redis veya Session'a kaydet
                if (userId != null)
                {
                    _redisHelper.SetCart(userId, cart);
                }
...
//        public CartController(IProductService productService, IOrderService orderService, RedisHelper redisHelper)
//        {
//            _productService = productService;
//            _orderService = orderService;
//            _redisHelper = redisHelper;
...
//        [HttpPost]
//        public ActionResult CompleteOrderUser(CheckoutViewModel model)
//        {
//            // Login olanlar için 
//            return RedirectToAction("OrderCompleted");
//        }
